...
def EEGSym(input_time=3000, fs=128, ncha=8, filters_per_branch=8,
    ...
    if residual:
        ...
        ch_merg = Conv3D(kernel_size=(division, 1, ncha),
                        filters=int(filters_per_branch * len(scales_samples) / 4),
                        groups=int(filters_per_branch * len(scales_samples) / 8),
                        use_bias=False,
    ...
    t_merg = Conv3D(kernel_size=(1, input_samples // 64, 1),
                    filters=int(filters_per_branch * len(scales_samples) / 4) * 2,
                    groups=int(filters_per_branch * len(scales_samples) / 4),
                    use_bias=False,
    ...
    for _ in range(4):
        ...
        output_temp = Conv3D(kernel_size=(1, 1, 1),
                             filters=int(filters_per_branch * len(scales_samples) / 2),
                             use_bias=False,
                             strides=(1, 1, 1),
                             kernel_initializer='he_normal',
